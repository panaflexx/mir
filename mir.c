...
/* Unload a module that was MIR_load_module'd: also drop it from modules_to_link
   (MIR_remove_module does not).  Frees IR; published code must be managed
   separately via isolation / _MIR_free_code.  Item table cleanup is done by
   remove_module (same as MIR_remove_module). */
void MIR_unload_module (MIR_context_t ctx, MIR_module_t m) {
  if (m == NULL) return;
  if (m == curr_module)
    MIR_get_error_func (ctx) (MIR_finish_error, "unload when module %s is not finished", m->name);
  if (m == &environment_module)
    MIR_get_error_func (ctx) (MIR_wrong_param_value_error, "MIR_unload_module: environment module");
  /* Remove from modules_to_link (loaded-but-not-yet-linked or leftover slots) */
...
void _MIR_compact_item_tab (MIR_context_t ctx) {
  ...
  for (m = DLIST_HEAD (MIR_module_t, all_modules); m != NULL; m = DLIST_NEXT (MIR_module_t, m)) {
...
struct machine_code_ctx {
  ...
  VARR (code_holder_t) * isolated_holders; /* per-isolation-session code pages */
...
static uint8_t *publish_code_isolated (MIR_context_t ctx, const uint8_t *code, size_t code_len) {
  ...
  if (cur_isolated_idx >= 0) {
    ...
    ch_ptr->free = (uint8_t *) ((uint64_t) (ch_ptr->free + 15) / 16 * 16); /* align */
    if (ch_ptr->free + code_len <= ch_ptr->bound) return add_code (ctx, ch_ptr, code, code_len);
  }
  /* Allocate a fresh code holder */
  {
    ...
    vlen = VARR_LENGTH (code_holder_t, isolated_holders);
    ...
    return add_code (ctx, VARR_ADDR (code_holder_t, isolated_holders) + cur_isolated_idx, code,
                     code_len);
  }
}
...
uint8_t *_MIR_publish_code (MIR_context_t ctx, const uint8_t *code,
  ...
  if (isolating) return publish_code_isolated (ctx, code, code_len);
...
void *_MIR_get_recycled_thunk (MIR_context_t ctx) {
  if (VARR_LENGTH (void_ptr_t, thunk_recycle) > 0) return VARR_POP (void_ptr_t, thunk_recycle);
  return NULL;
}
